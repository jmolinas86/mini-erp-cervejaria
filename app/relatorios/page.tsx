...
export default async function Relatorios({ searchParams }: { searchParams: Promise<{ aba?: string }> }) {
  ...
  return <AppShell active="relatorios" userEmail={email} contextLabel="Relatórios" contextCurrent="Custos e operação"><main className="page-shell reports-page"><section className="page-content">
    <div className="page-header"><div><p className="eyebrow">Fase 9 · Gestão</p><h1>Relatórios</h1><p className="intro">Acompanhe custos reais, produção e situação do estoque.</p></div><div className="page-header-actions"><Link className="button secondary" href={`/relatorios/exportar?tipo=${aba}`}>Baixar CSV</Link><ImprimirRelatorio /></div></div>
