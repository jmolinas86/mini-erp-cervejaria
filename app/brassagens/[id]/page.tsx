...
type Lote = { id: string; id_item: string; codigo_lote: string; recebido_em: string | null; validade: string | null };
...
export default async function BrassagemDetalhe({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  const [{ data: receita }, { data: itens }, { data: saldos }, { data: lotes }] = await Promise.all([
    ...
    supabase.from("lotes_itens").select("id,id_item,codigo_lote,recebido_em,validade").order("recebido_em", { ascending: true })
  ]);
  ...
  return <AppShell active="producao" userEmail={email} contextLabel="Produção" contextCurrent={registro.numero_brassagem}><main className="page-shell batch-page"><section className="page-content">
    ...
    <section className="panel-card"><div className="section-header"><div><p className="eyebrow">Consumo real</p><h2>Insumos da brassagem</h2><p className="section-help">O lote mais antigo com saldo é sugerido automaticamente. Confirme apenas se o lote e a quantidade estiverem corretos.</p></div><span className="status-pill configured">{listaConsumos.length} itens previstos</span></div><div className="batch-consumption-list">{listaConsumos.length ? listaConsumos.map((consumo) => { const item = itemPorId.get(consumo.id_item); const quantidadePrevista = numero(consumo.quantidade_prevista); const lotesDoItem = listaSaldos.filter((saldo) => saldo.id_item === consumo.id_item && (numero(saldo.quantidade_saldo) > 0 || saldo.id_lote === consumo.id_lote)).sort((a, b) => { const dataA = lotePorId.get(a.id_lote ?? "")?.recebido_em ?? "9999-12-31"; const dataB = lotePorId.get(b.id_lote ?? "")?.recebido_em ?? "9999-12-31"; return dataA.localeCompare(dataB); }); const loteAutomatico = lotesDoItem.find((lote) => numero(lote.quantidade_saldo) >= quantidadePrevista) ?? lotesDoItem[0] ?? null; const loteSelecionado = lotesDoItem.find((lote) => lote.id_lote === consumo.id_lote) ?? loteAutomatico; const quantidadeExibida = consumo.quantidade_real === null ? quantidadePrevista : numero(consumo.quantidade_real); const saldoSelecionado = numero(loteSelecionado?.quantidade_saldo); const bloqueado = !loteSelecionado || saldoSelecionado < quantidadeExibida; return <div className="batch-consumption-row" key={consumo.id}><div className="batch-consumption-info"><strong>{item?.nome ?? "Item removido"}</strong><span>{rotuloEtapa(consumo.etapa)} · quantidade da receita {decimal(quantidadePrevista)} {item?.codigo_unidade ?? ""}</span></div>{!encerrada ? <ConfirmarConsumo idConsumo={consumo.id} idBrassagem={id} idLote={loteSelecionado?.id_lote ?? ""} loteLabel={loteSelecionado?.codigo_lote ?? "Sem lote disponível"} saldoLabel={loteSelecionado ? `Saldo disponível: ${decimal(loteSelecionado.quantidade_saldo)} ${loteSelecionado.codigo_unidade ?? item?.codigo_unidade ?? ""}` : "Cadastre uma entrada de estoque para confirmar"} quantidade={quantidadeExibida} unidade={item?.codigo_unidade ?? "un"} confirmado={consumo.quantidade_real !== null} bloqueado={bloqueado} /> : <div className="batch-consumption-confirmed"><strong>{loteSelecionado?.codigo_lote ?? "Lote não informado"}</strong><span>{decimal(quantidadeExibida)} {item?.codigo_unidade ?? ""} · {consumo.quantidade_real === null ? "Pendente" : "Baixado"}</span></div>}</div>; }) : <div className="empty-state"><strong>Nenhum insumo foi copiado da receita.</strong><span>Edite a receita antes de abrir a brassagem.</span></div>}</div></section>
