...
export default async function EstoquePage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; error?: string; message?: string }> }) {
  ...
  const [{ data: saldos, error: erroSaldos }, { data: itens }, { data: fornecedores }, { data: lotes }, { data: movimentos }] = await Promise.all([
    ...
    <AppShell active="estoque" userEmail={email}>
      <main className="page-shell">
        <section className="page-content">
          ...
          <section className="panel-card" id="nova-entrada"><div className="section-header"><div><p className="eyebrow">Entrada de compra</p><h2>Novo lote</h2></div><span className="status-pill configured">Saldo atualizado ao salvar</span></div><form className="form-grid" action={registrarEntrada}><label className="form-field"><span>Item *</span><select name="id_item" required defaultValue=""><option value="" disabled>Selecione o item</option>{listaItens.map((item) => <option value={item.id} key={item.id}>{item.nome} · {item.codigo_unidade}</option>)}</select></label><label className="form-field"><span>Fornecedor</span><select name="id_fornecedor" defaultValue=""><option value="">Não informado</option>{listaFornecedores.map((fornecedor) => <option value={fornecedor.id} key={fornecedor.id}>{fornecedor.nome}</option>)}</select></label><label className="form-field"><span>Código do lote *</span><input name="codigo_lote" required placeholder="Ex.: MAL-2609" /></label><label className="form-field"><span>Quantidade *</span><input name="quantidade" type="number" min="0.001" step="0.001" required placeholder="0,000" /></label><label className="form-field"><span>Custo unitário *</span><input name="custo_unitario" type="number" min="0" step="0.0001" required placeholder="R$ 0,00" /></label><label className="form-field"><span>Recebido em *</span><input name="recebido_em" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} /></label><label className="form-field"><span>Validade</span><input name="validade" type="date" /></label><label className="form-field form-field-wide"><span>Observações</span><textarea name="observacoes" rows={2} placeholder="Nota fiscal, condição de pagamento ou localização" /></label><div className="form-actions form-field-wide"><button className="button primary" type="submit">Salvar entrada</button></div></form></section>

          <section className="panel-card" id="nova-movimentacao"><div className="section-header"><div><p className="eyebrow">Baixa e ajustes</p><h2>Registrar movimentação</h2></div><span className="status-pill pending">Saídas validam o saldo do lote</span></div><form className="form-grid" action={registrarMovimentacao}><label className="form-field"><span>Lote *</span><select name="id_lote" required defaultValue=""><option value="" disabled>Selecione o lote</option>{listaLotes.map((lote) => <option value={lote.id} key={lote.id}>{lote.codigo_lote} · {nomeItem.get(lote.id_item) ?? "Item"}</option>)}</select></label><label className="form-field"><span>Tipo *</span><select name="tipo_movimentacao" required defaultValue="saida_manual"><option value="saida_manual">Saída manual</option><option value="perda">Perda</option><option value="entrada_manual">Entrada manual</option><option value="ajuste_entrada">Ajuste de entrada</option><option value="ajuste_saida">Ajuste de saída</option></select></label><label className="form-field"><span>Quantidade *</span><input name="quantidade" type="number" min="0.001" step="0.001" required placeholder="0,000" /></label><label className="form-field"><span>Data *</span><input name="ocorrido_em" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} /></label><label className="form-field"><span>Custo unitário</span><input name="custo_unitario" type="number" min="0" step="0.0001" placeholder="Usa o custo do lote" /></label><label className="form-field form-field-wide"><span>Motivo / observações</span><textarea name="observacoes" rows={2} placeholder="Ex.: quebra, inventário ou uso fora da produção" /></label><div className="form-actions form-field-wide"><button className="button secondary" type="submit">Registrar movimentação</button></div></form></section>
