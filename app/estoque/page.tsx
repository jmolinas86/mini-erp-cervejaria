        ...
        <section className="page-content">
          ...
          <section className="panel-card">
            <div className="section-header">
              <div><p className="eyebrow">Posição atual</p><h2>Saldo por lote</h2></div>
              <form className="filter-form" method="get">
                <input name="q" defaultValue={params.q ?? ""} placeholder="Buscar item ou lote" />
                <select name="status" defaultValue={params.status ?? ""}><option value="">Todos os status</option><option value="ok">OK</option><option value="abaixo_minimo">Abaixo do mínimo</option><option value="vazio">Vazio</option></select>
                <button className="button secondary" type="submit">Filtrar</button>
              </form>
            </div>
            <p className="section-help">Edite o código, a validade ou o saldo desejado de qualquer lote. Alterações de quantidade ficam registradas como ajuste no histórico.</p>
            <div className="table-wrap"><table><thead><tr><th>Item</th><th>Lote</th><th>Validade</th><th>Saldo</th><th>Mínimo</th><th>Custo atual</th><th>Status</th><th>Ações</th></tr></thead><tbody>{saldosFiltrados.map((saldo) => {
              const item = listaItens.find((registro) => registro.id === saldo.id_item);
              const lote = saldo.id_lote ? listaLotes.find((registro) => registro.id === saldo.id_lote) : null;
              return <tr key={`${saldo.id_item}-${saldo.id_lote ?? "sem-lote"}`}>
                <td><strong>{item?.codigo_item ?? "—"} · {saldo.nome_item}</strong><span>{item?.codigo_grupo ?? saldo.categoria}</span></td>
                <td>{saldo.codigo_lote ?? "Sem lote"}</td>
                <td>{dataCurta(saldo.validade)}</td>
                <td><strong>{decimal(saldo.quantidade_saldo, 3)} {saldo.codigo_unidade}</strong></td>
                <td>{decimal(saldo.estoque_minimo, 3)} {saldo.codigo_unidade}</td>
                <td>R$ {decimal(saldo.custo_unitario, 4)}</td>
                <td><span className={`table-pill ${saldo.status_estoque}`}>{saldo.status_estoque === "abaixo_minimo" ? "Abaixo do mínimo" : saldo.status_estoque === "vazio" ? "Vazio" : "OK"}</span></td>
                <td>{lote ? <details className="row-details"><summary>Editar lote</summary><form className="details-form" action={editarLote}><input type="hidden" name="id_lote" value={lote.id} /><label className="form-field"><span>Código do lote *</span><input name="codigo_lote" defaultValue={lote.codigo_lote} required maxLength={80} /></label><label className="form-field"><span>Validade</span><input name="validade" type="date" defaultValue={lote.validade ?? ""} /></label><label className="form-field"><span>Saldo desejado ({saldo.codigo_unidade}) *</span><input name="quantidade_desejada" type="number" min="0" step="0.001" defaultValue={numero(saldo.quantidade_saldo)} required /><small>Saldo atual: {decimal(saldo.quantidade_saldo, 3)} {saldo.codigo_unidade}. A diferença será lançada como ajuste.</small></label><button className="button primary" type="submit">Salvar lote</button></form></details> : <span>—</span>}</td>
              </tr>;
            })}</tbody></table></div><p className="table-footnote">Mostrando {saldosFiltrados.length} de {listaSaldos.length} posições.</p>
          </section>
