...
export default async function ReceitasPage({ searchParams }: { searchParams: Promise<{ receita?: string; versao?: string; error?: string; message?: string }> }) {
  ...
  return <AppShell active="receitas" userEmail={email} contextLabel="Receitas" contextCurrent={receitaNova ? "Nova receita" : receitaSelecionada.nome}><main className="page-shell recipe-editor-page"><section className="page-content">
    <div className="recipe-editor-header"><div><div className="recipe-title-line"><h1>{receitaNova ? "Nova receita" : receitaSelecionada.nome}</h1><span className="recipe-draft-pill">● Rascunho</span></div><p className="intro">Cadastre os parâmetros, ingredientes e etapas de produção</p></div><div className="recipe-editor-actions"><Link className="button secondary" href="/receitas">Cancelar</Link>{receitaSelecionada && versaoSelecionada ? <Link className="button primary" href={`/brassagens?receita=${receitaSelecionada.id}&versao=${versaoSelecionada.id}`}>▶ &nbsp; Iniciar brassagem</Link> : null}<button className="button secondary" type="submit" form="recipe-info-form" disabled={!receitaNova}>Salvar rascunho</button><button className="button primary" type="submit" form="recipe-info-form" disabled={!receitaNova}>▣ &nbsp; Salvar receita</button></div></div>
